class FreecellGame {
  ...
  _addCard(destPile, srcPile) {
    let dest = null;
    let srcCards = this._getPile(srcPile.type)[srcPile.index].slice(
      srcPile.cardIndex
    );
  ...
  attemptAutoMove(srcPile) {
    ...
    switch (srcPile.type) {
      case "cascade":
        oPiles = this.cascade[srcPile.index];
        src = oPiles[srcPile.cardIndex];
        break;
      case "open":
        oPiles = this.open[srcPile.index];
        src = oPiles[0];
        break;
      case "foundation":
        oPiles = this.foundation[srcPile.index];
        src = oPiles[srcPile.cardIndex];
        break;
      default:
        break;
  ...
  getValidFoundation(srcPile) {
    ...
    for (let i = 0; i < this.foundation.length; i++) {
      ...
      if (fPile.length != 0) {
        if (
          fPile[fPile.length - 1].value == src.value - 1 &&
          fPile[fPile.length - 1].getSuit() == src.getSuit()
        ) {
          return i;
        }
      } else {
  ...
  isValidMove(srcPile, destPile) {
    ...
    if (cardSeq.length > this._numCanMove(destPile)) {
      return false;
    }
  ...
  static _isStackable(underCard, overCard) {
    return (
      overCard.value == underCard.value + 1 &&
